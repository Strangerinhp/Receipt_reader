...
const tabList = [
  { label: "TẢI LÊN", icon: <FileUploadIcon sx={{ mr: 0.5 }} /> },
  { label: "KIỂM TRA", icon: <FactCheckIcon sx={{ mr: 0.5 }} /> },
  { label: "ĐÃ LƯU", icon: <SummaryIcon sx={{ mr: 0.5 }} /> },
];
...
const Tabbar = () => {
  ...
  const handleChange = (event, newValue) => {
    if (newValue === 0 || (newValue === 1 && ocrCtx.draft) || (newValue === 2 && ocrCtx.savedId)) {
      ...
      <Grid item xs={12} container justifyContent="center" flexWrap="wrap">
        ...
        >
          {tabList.map((tab, index) => (
            <Tab
              key={index}
              disabled={(index === 1 && !ocrCtx.draft) || (index === 2 && !ocrCtx.savedId)}
